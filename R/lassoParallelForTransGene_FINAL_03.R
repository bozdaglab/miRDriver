...
lassoParallelForTransGene <-
  function(ncores = 2,
    ...
    processLassoCtr <- function(numCounter, Lx, Ly, gene_id) {
      tryCatch({
        ...
        cat(
          ...
          file = "~/mirDriverFold/ERROR_FILES/Step4_LassoErrors.txt",
          sep = "\n",
          append = TRUE
        )
        return()
    ...
    processUpDownGeneFiles <- function(fileName, processLassoCtr) {
      load(paste0(
        "~/mirDriverFold/miRDriver_Step4/TransGenePredFile/",
        fileName,
        sep = ""
      ))
      ...
      GenePred <-
        subset(GenePred, select = which(!duplicated(names(GenePred))))##checking on duplicated colnames
      if (ncol(GenePred) > 2)
      {
        ...
        save(
          min_Coeff,
          file = paste0(
            "~/mirDriverFold/miRDriver_RESULTS/LassoMinCoeff/",
            gene_id,
            ".Rda"
          )
        )
      ...
      } else
      {
        save(GenePred,
             file = paste0(
               "~/mirDriverFold/ERROR_FILES/Step4_Lasso_",
               gene_id,
               ".Rda"
             ))##this will give us genes on which lasso didn't run
      }
    ...
    if (parallel == TRUE) {
      ...
      cl <-
        makeCluster(mc <- getOption("cl.cores", ncores), type = "FORK")
      ...
      parallel::clusterExport(
        ...
        envir = environment()
      )#envir needed to be correct, export from global aswell as local
      print("cluster making done")


      fileNames <-
        list.files("~/mirDriverFold/miRDriver_Step4/TransGenePredFile/",
                   pattern = "*.Rda") ##we have 3518 GenePred files in this folder
    ...
    processMirGene <- function(fileNames, nonZeroTimes) {
      ...
      nx <-
        sqldf(
          "select genes_min,predictors_min,count(coefficients_min) coeff_min_count, avg(coefficients_min)
          avg_coeff from min_Coeff where coefficients_min <> 0 group by genes_min,predictors_min "
        )
    ...
    LASSOfileNames <-
      list.files("~/mirDriverFold/miRDriver_RESULTS/LassoMinCoeff/", pattern = "*.Rda")#3117 genes
