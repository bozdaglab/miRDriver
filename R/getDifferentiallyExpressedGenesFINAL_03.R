...
getDifferentiallyExpressedGenes <-
  function(ncore = 2,
    ...
    fileNames <-
      list.files("~/mirDriverFold/miRDriver_Step1/Regionwise_Gistic_Files",
                 pattern = "*.txt")
    ...
    #So all other DE gene related study should be on these 365 common samples.
    ...
    processEdgeR <- function(fileName) {
      tryCatch({
        ...
        y <- estimateDisp(y, design, robust = TRUE)#####FAULTY
        ...
        write.table(
          UPDEgene,
          paste(
            "~/mirDriverFold/miRDriver_Step2/UpDownGeneLibrary/UPgene_",
            fileNameOnly,
            ".txt",
            sep = ""
          )
        ...
        write.table(
          DNDEgene,
          paste(
            "~/mirDriverFold/miRDriver_Step2/UpDownGeneLibrary/DOWNgene_",
            fileNameOnly,
            ".txt",
            sep = ""
          )
        ...
        cat(
          ...
          file = "~/mirDriverFold/ERROR_FILES/Step2_DEanalysisError.txt",
          sep = "\n",
          append = TRUE
        )
    ...
    if (parallel == TRUE) {
      ...
      parallel::clusterExport(cl = cl, c("commonSample", "Z"), envir = environment())#envir needed to be correct, export from global aswell as local
