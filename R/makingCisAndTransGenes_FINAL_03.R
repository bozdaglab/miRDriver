...
makingCisAndTransGenes <-function(Genes_coord_bedfile, gistic_bedfile) {
    ##making necessary directories
    dir.create("~/mirDriverFold/miRDriver_RESULTS")
    dir.create("~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES")
    dir.create("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files")
    ############## Reading up/down genes files for a gistic region
    fileNames <-list.files("~/mirDriverFold/miRDriver_Step2/UpDownGeneLibrary/",pattern = "*.txt")
    ...
    MakecistransFile <- function(i) {
        print(i)
        upgene <-read.table(paste0("~/mirDriverFold/miRDriver_Step2/UpDownGeneLibrary/",fileNames[i]),sep = " ",header = TRUE)
        ...
        if (gistic_bedfile == FALSE) {
            load("~/mirDriverFold/miRDriver_Step1/GisticResults/gistic_bedfile.rda")
        ...
        if (nrow(Region) == 0) {
            ...
            Region <- bedr.sort.region(Region, check.chr = FALSE)
            Gistic_Genes <-bedr.join.region(Genes_coord_bedfile,Region,report.n.overlap = TRUE,check.chr = FALSE)
            cis_genes <-data.frame(Gistic_Genes$gene)
            if (nrow(cis_genes) == 0) {
                print("This region has no cis genes")
                trans_genes <- genes
                colnames(trans_genes) <- "gene_id"
                save(trans_genes,file = paste0("~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES/trans_",fileNames[i],".Rda"))
            } else{
    ...
    save(Count_Cis_Trans, file = "~/mirDriverFold/miRDriver_RESULTS/Count_Cis_Trans.Rda")
    ...
    for (i in seq_along(fileNames1)){
        ...
        if (is.element(fileNamesD, fileNames2)) {
            file.remove(paste0("~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES/",fileNames1[i]))
        } else
        {
        }
    }
    ...
    processUpDownGeneFiles <- function(fileNameUpDn) {
        ...
        if (str_sub(fileNameUpDn, 1, 3) == "cis") {
            load(paste0("~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES/",fileNameUpDn))
            load(paste0("~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES/",gsub(".*cis", "trans", fileNameUpDn)))
        } else if (str_sub(fileNameUpDn, 1, 5) == "trans")
        {
            ...
            {
                load(paste0("~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES/",gsub(".*trans", "cis", fileNameUpDn)))
            } else
            {
        ...
        dir.create(paste0("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files/",RegionNameOnly))
        save(cis_genes,file = paste0("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files/",RegionNameOnly,"/cis_genes.Rda"))
        save(trans_genes,file = paste0("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files/",RegionNameOnly,"/",transfile,".Rda"))
    }
    ##### MAIN PROGRAM ############################################################
    fileNames <-list.files("~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES", pattern = "*.Rda")
    ...
    processUpDownGeneFiles2 <- function(fileName) {
        trans_gene <- NULL
        genefiles <- NULL
        genefiles<-list.files(paste0("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files/",fileName,"/"))
        ...
        if (is.element(transfiles, genefiles)) {
            load(paste0("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files/",fileName,"/",transfiles))
            transgene1 <- trans_genes
            colnames(transgene1) <- "gene_id"
            if (is.element(transfiles_new, genefiles)) {
                load(paste0("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files/",fileName,"/",transfiles_new))
            ...
            save(trans_genes,file = paste0("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files/",fileName,"/trans_genes.Rda"))
        } else{
            load(paste0("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files/",fileName,"/",transfiles_new))
            transgene2 <- trans_genes
    ...
    folderNames <-list.files("~/mirDriverFold/miRDriver_Step2/Trans_Cis_Files/")
