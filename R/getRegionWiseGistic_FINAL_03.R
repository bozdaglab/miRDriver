...
getRegionWiseGistic1 <- function(humanGene,copyNumber,conf.lev,username,password,gistic_bedfile = c('TRUE', 'FALSE')){
    ...
    file <-
      paste0(
        "ftp://gpftp.broadinstitute.org/module_support_files/GISTIC/refgene/",
        humanGene
      )
    ...
    GISTIC_2.0.result <-
      run.analysis(
        ...
        refgene.file = file,
        seg.file = copyNumber,
        confidence.level = conf.lev
      )
    ##Download the results from GISTIC 2.0
    setwd("~/")
    GISTIC_2.0.out.files <-
      job.result.download.files(GISTIC_2.0.result,
                                "mirDriverFold/miRDriver_Step1/GisticResults/")
    
    ##Getting lesions file in miRDriver_Step1/GisticResults
    lesion_file <-
      list.files("~/mirDriverFold/miRDriver_Step1/GisticResults", pattern = "all_lesions")
    ##Processing region wise gistic files
    Gistic_lesions <-
      read.table(
        paste0("~/mirDriverFold/miRDriver_Step1/GisticResults/", lesion_file),
        sep = "\t",
        header = TRUE
      )
    ...
    indx <-
      which(Gistic_lesions == '1' | Gistic_lesions == '2', arr.ind = TRUE)
    val <-
    ...
    BR <-
      Gistic_lesions[, c(1:9, ncol(Gistic_lesions) - 1):(ncol(Gistic_lesions))]#Important Region-Patients group with only important cols
    ...
    for (j in 1:nrow(BR)) {
      ...
      myfile <-
        file.path(
          paste0(
            "~/mirDriverFold/miRDriver_Step1/Regionwise_Gistic_Files/",
            BR_Peak,
            ".txt"
          )
        )
      ...
      for (i in 1:length(BR_Aberrated))
        writeLines(paste(BR_Aberrated[i], ",1,", "aberrated"), fileConn)
      for (i in 1:length(BR_NonAberrated))
      ...
      if (gistic_bedfile == TRUE) {
        ...
        gistic_region <-
          gistic_region[-grep("CN values", gistic_region$Unique.Name), ]
        ...
        Gistic_Bedfile <-
          data.frame(Gistic_Bedfile)## for this dataframe is necessary
    ...
    if (gistic_bedfile == TRUE) {
      return(
        paste0(
          "gistic_bedfile.rda is created in folder 'miRDriver_Step1/GisticResults' and regionwise gistic files are ready in folder 'miRDriver_Step1/Regionwise_Gistic_Files'"
        )
      )
    }
    if (gistic_bedfile == FALSE) {
      return(list(
        paste0(
          "regionwise gistic files are ready in folder 'mirDriverFold/miRDriver_Step1/Regionwise_Gistic_Files'"
        )
      ))
    }
  }
...
getRegionWiseGistic2 <-
  function(gisticfile,
    ...
    Gistic_lesions <-  Gistic_lesions[1:nrow(Gistic_lesions), ]
    indx <-
      which(Gistic_lesions == '1' | Gistic_lesions == '2', arr.ind = TRUE)
    val <-
      tapply(names(Gistic_lesions)[indx[, 2]], indx[, 1], FUN = toString)
    ...
    Gistic_lesions$NonAberrated_patients <-
      ifelse(seq_len(nrow(Gistic_lesions)) %in% names(val1), val1, NA)
    BR <-
      Gistic_lesions[, c(1:9, ncol(Gistic_lesions) - 1):(ncol(Gistic_lesions))]#Impotrtant Region-Patients group with only important cols
