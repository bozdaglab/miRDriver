...
x <-
    read.table(
        file.path(
            ...
            "miRDriver_Step2",
            "UpDownGeneLibrary",
            "UPgene_Amplification Peak  9.txt"
        ),
        sep = "",
        header = TRUE
    )
...
# <a id = "Running-miRDriver-at-a-glance"></a> Running miRDriver at a glance
```{r}
...
getRegionWiseGistic(
    gisticfile = system.file("extdata", "Dummy.gistic.lesions.txt", package =
                                 "miRDriver"),
    gistic_bedfile = TRUE,
    mirdirectory = mirdirectory
)
