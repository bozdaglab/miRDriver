...
```{r}
getRegionWiseGistic(gisticfile = system.file("extdata", "Dummy.gistic.lesions.txt", package="miRDriver"), gistic_bedfile = TRUE, mirdirectory = tempdir())
```
...
```{r}
load(file.path(tempdir(), "mirDriverFold", "miRDriver_Step1", "GisticResults", "gistic_bedfile.rda"))
region[1:5,]

```

```{r}
x <- data.frame(read.table(file.path(tempdir(), "mirDriverFold", "miRDriver_Step1", "Regionwise_Gistic_Files", "Amplification Peak  7.txt"), sep = "\t", header = TRUE))
...
```{r warning=FALSE}
getDifferentiallyExpressedGenes(RNACount = Dummy_RnaSeq_Count_Data, mirdirectory = tempdir())
```
...
```{r}
x <- read.table(file.path(tempdir(), "mirDriverFold", "miRDriver_Step2", "UpDownGeneLibrary", "UPgene_Amplification Peak  9.txt"), sep = "", header = TRUE)
x[1:3,]
```
...
```{r}
load(file.path(tempdir(), "mirDriverFold", "miRDriver_Step1", "GisticResults", "gistic_bedfile.rda"))
makingCisAndTransGenes(Genes_coord_bedfile = Genes_coord, gistic_bedfile=region, mirdirectory=tempdir())
```
...
```{r}
load(file.path(tempdir(), "mirDriverFold", "miRDriver_Step2", "Trans_Cis_Files", "Amplification Peak  1", "trans_genes.Rda"))
trans_genes[1:3,]
...
options(mc.cores=2)
load(file.path(tempdir(), "mirDriverFold", "miRDriver_Step1", "GisticResults", "gistic_bedfile.rda"))
gatherCisGenes(ncore = 2, mirna_bedfile=mirna_bedfile, gistic_bedfile=region, mirdirectory=tempdir())
```
...
```{r}
load(file.path(tempdir(), "mirDriverFold", "miRDriver_Step3", "gene_all_DEcis", "ENSG00000036828.Rda"))
TC
message("For this trans gene we do not have any cis gene")
```
```{r}
load(file.path(tempdir(), "mirDriverFold", "miRDriver_Step3", "gene_all_mirna", "ENSG00000124205.Rda"))
GM[1:3,]
```
...
```{r}
getTransGenePredictorFile(methylationData = dummy_methylationData10, RSeqData = dummy_RSeqData10_200,                           CNVData = dummy_CNVData10, TFData = dummy_TFData, mirnaData =                                        dummy_mirnaData10, mirdirectory = tempdir())
```

Output: The output for this function can be found in the mirdirectory in mirDriverFold/miRDriver_Step4/TransGenePredFile.

```{r}
load(file.path(tempdir(), "mirDriverFold", "miRDriver_Step4", "TransGenePredFile", "ENSG00000108231.Rda"))
GenePred[1:3,]
```
...
```{r}
lassoParallelForTransGene(ncore = 2, numCounter = 10,Nfolds = 3,nonZeroPercent = 20, mirdirectory =                           tempdir())
```
...
```{r}
load(file.path(tempdir(), "miRDriverFold", "miRDriver_RESULTS", "LassoMinCoeff", "ENSG00000088387.Rda"))
min_Coeff[1:3,]
```

```{r}
load(file.path(tempdir(), "miRDriverFold", "miRDriver_RESULTS", "Final_gene_mirna_network.Rda"))
