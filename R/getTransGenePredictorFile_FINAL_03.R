  ...
  function(ncores = 2,
    ...
    processEachGene <- function(GeneFile) {
      tryCatch({
        ...
        mirna <-
          data.frame(colnames(MG[, -1]))## getting mirnas from mirnaData
        ...
        } else
        {
          ...
          common <-
            intersect(TC$cis_gene, all_genes$gene)## getting cis genes for rnaseq
          ...
          Geneid <- str_sub(GeneFile, end = -5)##trans gene
          ...
          if (is.element(Geneid, colnames(RSeq))) {
            Y <-
              cbind(RSeq[, 1, drop = FALSE], RSeq[, ..Geneid])## Rnaseq for trans gene as response variable
          } else{
          ...
          df <-
            Reduce(function(...)
              merge(..., by = "sample", x.all = TRUE), dfs) #1052 unique samples
          df <- remove_empty(df)
        ...
        cat(
          ...
          file = "~/mirDriverFold/ERROR_FILES/Step4_transGenePredictorError.txt",
          sep = "\n",
          append = TRUE
        )
      }, finally = {
    ...
    if (parallel == TRUE) {
      ...
      #################
      GeneFileNames <-
        list.files("~/mirDriverFold/miRDriver_Step3/gene_all_mirna/", pattern = "*.Rda")
