...
getRegionWiseGistic <-function(gisticfile,gistic_bedfile = c('TRUE', 'FALSE'),mirdirectory="~/") {

    dir.create(mirdirectory)
    dir.create(paste0(mirdirectory,"/mirDriverFold"))
    dir.create(paste0(mirdirectory,"/mirDriverFold/miRDriver_Step1"))
    dir.create(paste0(mirdirectory,"/mirDriverFold/miRDriver_Step1/GisticResults"))
    dir.create(paste0(mirdirectory,"/mirDriverFold/miRDriver_Step1/Regionwise_Gistic_Files"))
    ...
    for (j in seq_len(nrow(BR))) {
        ...
        myfile <-file.path(paste0(mirdirectory,"/mirDriverFold/miRDriver_Step1/Regionwise_Gistic_Files/",BR_Peak,".txt"))
    ...
    if (gistic_bedfile == TRUE) {
        return(paste0("gistic_bedfile.rda is created in mirdirectory under 'mirDriverFold/miRDriver_Step1/GisticResults' and regionwise gistic files are ready in folder 'miRDriver_Step1/Regionwise_Gistic_Files'"))
    }
    if (gistic_bedfile == FALSE) {
        return(list(paste0("regionwise gistic files are ready in mirdirectory under 'mirDriverFold/miRDriver_Step1/Regionwise_Gistic_Files'")))
    }
}
