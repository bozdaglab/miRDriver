...
makingCisAndTransGenes <-
  function(Genes_coord_bedfile, gistic_bedfile) {
    ...
    fileNames <-
      list.files("~/mirDriverFold/miRDriver_Step2/UpDownGeneLibrary/",
                 pattern = "*.txt")
    Count_Cis_Trans <- data.frame("", "", "")
    colnames(Count_Cis_Trans) <-
      c("Gistic_Region", "cis_genes_count", "trans_genes_count")

    i <- as.integer(1:length(fileNames))
    MakecistransFile <- function(i) {
      print(i)
      upgene <-
        read.table(
          paste0(
            "~/mirDriverFold/miRDriver_Step2/UpDownGeneLibrary/",
            fileNames[i]
          ),
          sep = " ",
          header = TRUE
      ...
      if (nrow(Region) == 0) {
        ...
        cis_genes <-
          data.frame(Gistic_Genes$gene) ### 2 genes are cis genes
        if (nrow(cis_genes) == 0) {
          ...
          save(
            trans_genes,
            file = paste0(
              "~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES/trans_",
              fileNames[i],
              ".Rda"
            )
          )
    ...
    fileNames1 <-
      list.files("~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES/", pattern = "*.Rda")
    ...
    for (i in 1:length(fileNames1))
    {
      ...
      if (is.element(fileNamesD, fileNames2)) {
        file.remove(paste0(
          "~/mirDriverFold/miRDriver_Step2/CIS_TRANS_FILES/",
          fileNames1[i]
        ))
      } else
      {
